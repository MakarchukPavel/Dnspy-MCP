...
public static class LiveDebugTools
{
    ...
    [Description("[DEBUG] Invoke an instance method or property getter on the paused frame's receiver and return the result — real func-eval via ICorDebug (RUNS target code, unlike debug_eval). expr = \"<root>.<member>[<TypeArgs>]([args])\", root = arg<i>/local<i>/'this'. A bare <member> (no parens) is a property getter (get_<member>) then a 0-arg method; <member>(...) calls a method; generic methods take explicit type args — e.g. \"arg0.GetTypedColumnValue<System.Guid>('UId')\", \"this.Plus(10)\", \"local0.Tag('x')\". Argument literals: integer (dec/0x-hex), true/false, null, or a quoted string. Overloads selected by (arg count, type-arg count, AND arg type — a string literal picks the (string) overload over a (SomeClass) one); resolves across base types. Runs on the paused thread with other threads SUSPENDED, timeout (default 2000ms) + abort. WARNING: runs target code — if it blocks on a lock another thread holds it can stall the target until the timeout; prefer debug_eval (passive) when a field/auto-property answers the question. Result decoded like debug_eval; a thrown exception returns {kind:'exception',type,message}; timeout returns {kind:'timeout'}. Not supported: non-literal/object args, nested-generic type args, value-type receivers, multi-hop receivers. Params: expr (required), frameIndex=0, timeoutMs=2000.")]
